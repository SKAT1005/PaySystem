...
from .views import GetBalanceView, GetHistoryView, TopupBalanceView, TransferBalanceView
...
class BalanceTests(TestCase):
    ...
    def test_transfer_balance_view(self):
        ...
        request = self.factory.post(
            "/transfer/", {"receiver_id": receiver.id, "amount": "25.00"}, format="json"
        )
        force_authenticate(request, user=self.user)
    ...
    def test_transfer_balance_view_invalid_receiver(self):
        request = self.factory.post(
            "/transfer/", {"receiver_id": 12345, "amount": "25.00"}, format="json"
        )
        force_authenticate(request, user=self.user)
        view = TransferBalanceView.as_view()
        response = view(request)
    ...
    def test_transfer_balance_view_invalid_amount(self):
        ...
        request = self.factory.post(
            "/transfer/",
            {"receiver_id": receiver.id, "amount": "invalid"},
            format="json",
        )
        force_authenticate(request, user=self.user)
        view = TransferBalanceView.as_view()
    ...
    def test_get_history_view(self):
        transaction = Transaction.objects.create(
            user=self.user, amount=Decimal("50.00"), type=TransactionType.TOPUP
        )
    ...
    def test_transfer_balance_service(self, mock_transfer_balance):
        ...
        mock_transfer_balance.assert_called_once_with(
            self.user, receiver, Decimal("25.00")
        )
    ...
    def test_transfer_balance_service_invalid_amount(self, mock_transfer_balance):
        ...
        mock_transfer_balance.side_effect = ValueError(
