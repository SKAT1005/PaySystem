...
class TopupBalanceView(views.APIView):
    ...
    def post(self, request):
        ...
        if not amount:
            return Response(
                {"error": "Не указана сумма."}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            return Response(
                {"error": "Неверный формат суммы."}, status=status.HTTP_400_BAD_REQUEST
            )
...
class TransferBalanceView(views.APIView):
    ...
    def post(self, request):
        ...
        if not receiver_id or not amount:
            return Response(
                {"error": "Не указаны данные получателя или сумма."},
